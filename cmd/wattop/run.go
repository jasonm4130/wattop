...
func (l *Loop) Run(ctx context.Context) {
	for {
		...
		if !l.sysAvailable {
			// soc.Sample either never started (Init failed at startup) or
			// has stopped blocking usefully; self-pace instead of hammering
			// the loop, and retry Init on the same backoff schedule Cycle
			// uses for a mid-run failure.
			select {
			case <-ctx.Done():
				return
...
// (via a state.SourceHealth entry) rather than aborting the cycle.
func (l *Loop) Cycle(ctx context.Context) state.Inputs {
	...
	if sysOK {
		...
		// already false on entry) falls back to the ticker-paced retry
		// path above.
		sys = l.lastSys
	}
